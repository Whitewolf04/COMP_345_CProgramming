...
std::vector<Territory*> Map::getAllTerritoriesP(){
    return ListOfNodesP;
}

void Map::convertToPointers(){

    for (int i = ListOfNodes->size()-1; i >= 0;i--){
        ListOfNodesP.push_back(new Territory());

    }
    for (int i = 0; i < ListOfNodes->size();i++){
        ListOfNodesP[i] = &ListOfNodes->at(i);
    }
}
...
ostream& operator<<(ostream& os, const Territory& t) {
    ...
    os << "Territory Number : " << t.T_num << " , Territory Name : " << t.T_name << " Belongs to Player#: "<< t.PlayerNumber <<
